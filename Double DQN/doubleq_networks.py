...
class DoubleQ_Network(object):
    def __init__(self, info):
        ...
        self.greedy_action = tf.argmax(self.q_values, axis=1, name="greedy_action")
        # Double Q
        self.one_hot_greedy_action = tf.one_hot(self.greedy_action, self.dim_a, dtype=tf.float32, name="1hot_greedy_action")
        self.target_q_double = tf.reduce_sum(self.target_q_values * self.one_hot_greedy_action, reduction_indices=[1], name="target_q_double")
        # Regular Q
    ...
    def target_q(self, inputs):
        ...
        return self.sess.run([self.target_q_double], feed_dict={
            self.inputs: inputs
        })
